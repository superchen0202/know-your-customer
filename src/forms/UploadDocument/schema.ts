...
const requiredSingleFile = (fieldName: string) =>
  union([instanceof_(File), literal(null)])
    ...
    .refine(
      (file): file is File => file instanceof File && checkInvalidFormatFileNumbers([file], acceptAttributes) === 0,
      {
...
export const schema = object({
  ...
  additionalDocs: array(
    instanceof_(File)
      .refine(
        (files): files is File =>
          files instanceof File && checkInvalidFormatFileNumbers([files], buildAcceptAttribute(ACCEPTED_FORMATS)) === 0,
        {
